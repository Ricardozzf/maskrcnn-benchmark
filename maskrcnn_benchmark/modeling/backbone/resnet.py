...
#ResNet-50-FPN-Adapt
ResNet50FPNAdaptStagesTo5 = tuple(
    StageSpec(index=i, block_count=c, return_features=r)
    for (i, c, r) in ((1, 3, True), (2, 3, True), (3, 3, True), (4, 3, True), (5, 3, True))
)
...
class ResNet(nn.Module):
    def __init__(self, cfg):
        ...
        for stage_spec in stage_specs:
            ...
            stage2_relative_factor = stage2_relative_factor if stage2_relative_factor <=8 else 4
...
class Bottleneck(nn.Module):
    def __init__(
        ...
        for l in [self.conv1, self.conv2, self.conv2_1, self.conv2_2, self.conv2_f, self.conv2_f1, self.conv3,]:
            nn.init.kaiming_uniform_(l.weight, a=1)
    ...
    def forward(self, x):
        ...
        #n_max1 = out.max()
        #n_max2 = identity.max()
        #w1 = out.max(2)[0].unsqueeze(2) / n_max1
        #h1 = out.max(3)[0].unsqueeze(3) / n_max1
        #w2 = identity.max(2)[0].unsqueeze(2) / n_max2
        #h2 = identity.max(3)[0].unsqueeze(3) / n_max2

        #out = out*w1/2 + out*h1/2 + identity
...
_STAGE_SPECS = Registry({
    ...
    "R-50-FPN-ADAPT": ResNet50FPNAdaptStagesTo5,
})
